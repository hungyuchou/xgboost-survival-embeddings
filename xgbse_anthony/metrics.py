...
def concordance_index_censored(y_true, survival, risk_strategy="mean", which_window=None, censored_weight=999):
    """
    ...
    for _, row in cind_df.iterrows():
        ...
        if row['e'] == False:
            samples_after_i = samples_after_i[samples_after_i["e"] == True]

        # counting total, concordant and tied pairs
        # count_pairs += samples_after_i.shape[0]
        count_pairs += len(samples_after_i[samples_after_i["e"] != row["e"]]) * censored_weight + len(samples_after_i[samples_after_i["e"] == row["e"]])
        concordant_pairs += ((samples_after_i["r"] < row["r"]) * (samples_after_i["e"] != row["e"]) * censored_weight).sum()
        tied_pairs += ((samples_after_i["r"] == row["r"]) * (samples_after_i["e"] != row["e"]) * censored_weight).sum()
